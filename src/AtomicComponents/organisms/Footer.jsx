...
const Footer = () => {
  ...
  return (
    <footer className="h-auto py-6 px-standard-x bg-gray-main text-white">
      {/* Sponsor Container */}
      <div id="sponsor-container" className="flex justify-center py-standard-y">
      ...
      <div className="information-container py-2">
        ...
        <nav className="grid-container flex items-center justify-between">
          ...
          <div className="information grid grid-rows-[auto_auto]">
            <div className="row-start-1 grid grid-cols-2 gap-x-20 gap-y-10">
              {infoLinks.map(({ title, links }) => (
                <ul key={title}>
                  <h1 className="text-3xl text-lime-300 mb-3">{title}</h1>
                  {links.map(({ text, path }) => (
                    <li key={text} className={navHoverStyle}>
                      <Link to={path} className="text-h4">
                        {text}
                      </Link>
                    </li>
                  ))}
      ...
      <div className="flex justify-between pt-6">
        <div className="">
          <img src={Logo} className="w-32 bg-blue-500 rounded-full p-2" />
